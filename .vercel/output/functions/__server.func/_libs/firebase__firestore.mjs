import { _ as _isFirebaseServerApp, g as getApp, a as _getProvider, b as _registerComponent, r as registerVersion, S as SDK_VERSION$1 } from "./firebase__app.mjs";
import { C as Component } from "./firebase__component.mjs";
import { F as FirebaseError, a as getModularInstance, c as isSafari, e as getUA, f as getDefaultEmulatorHostnameAndPort, h as isCloudWorkstation, p as pingServer, d as deepEqual, j as createMockUserToken } from "./firebase__util.mjs";
...
class CompositeFilter extends Filter {
  ...
  matches(doc) {
    if (compositeFilterIsConjunction(this)) {
      return this.filters.find((filter) => !filter.matches(doc)) === void 0;
    } else {
      return this.filters.find((filter) => filter.matches(doc)) !== void 0;
    }
  }
...
function fromWatchChange(serializer, change) {
  ...
  if ("targetChange" in change) {
    ...
    const doc = MutableDocument.newFoundDocument(key, version2, createTime, data);
    ...
    const doc = MutableDocument.newNoDocument(key, version2);
...
class LocalDocumentsView {
  ...
  computeViews(transaction, docs, overlays, existenceStateChanged) {
    ...
    docs.forEach((_, doc) => {
      ...
      if (existenceStateChanged.has(doc.key) && (overlay === void 0 || overlay.mutation instanceof PatchMutation)) {
...
function firestoreClientGetDocumentsViaSnapshotListener(client, query, options2 = {}) {
  const deferred = new Deferred();
  client.asyncQueue.enqueueAndForget(async () => {
    const eventManager = await getEventManager(client);
    return executeQueryViaSnapshotListener(eventManager, client.asyncQueue, query, options2, deferred);
  });
  return deferred.promise;
}
...
function getFirestore(appOrDatabaseId, optionalDatabaseId) {
  ...
  const databaseId = typeof appOrDatabaseId === "string" ? appOrDatabaseId : optionalDatabaseId;
